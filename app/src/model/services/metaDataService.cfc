component output="false" {
	...
	public boolean function inheritsFrom(required component obj, required string inheritedFrom) {
		var metaData = getMetaData(obj);
		while (metaData.keyExists('extends')) {
			if (metaData.extends.keyExists('name') && metaData.extends.name == inheritedFrom) {
				return true;
			}
